...
import com.mysecurity.demo.util.PasswordEncoder;
...
public class MySecurityProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authenticate) throws AuthenticationException {
        ...
        //数据库中密码已是加密后的数据，故对用户输入的密码也须加密后再比较
        if(!password.equals(new PasswordEncoder().encode(token.getCredentials().toString()))) {
